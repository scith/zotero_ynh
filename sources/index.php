...
$webdav_url=( isset($_SERVER['HTTPS']) ? 'https' : 'http') . "://" . $_SERVER['HTTP_HOST']  . str_replace('?'.$_SERVER['QUERY_STRING'],'',$_SERVER['REQUEST_URI']) . "zotero/";

?>
<h3>
    Total size of stored attachments: <u><?php echo format_size( foldersize( get_real_path( $data_dir ) ) ) ?></u>
    &nbsp; &nbsp; &nbsp; &nbsp;
    WebDAV URL: <a href="<?php echo $webdav_url ?>"><?php echo $webdav_url ?></a> with username "<?php echo $basicauthname ?>" and password "<?php echo $basicauthpass ?>"
</h3>
<?php
